...
4. (Firefoxの場合: `about:addons` → 該当の拡張機能 → 「権限（Permissions）」タブから **「ユーザーのコンピューター上のローカルファイルへのアクセス」** をオンにします。これはFirefox 153で新設された独立した権限で、以前は他のホスト権限に含まれていました。`about:debugging` から読み込んだ**一時的な**アドオンでテストしている場合、この権限をオンにするだけでは反映されません — Firefoxを再起動するだけでは不十分で、一時的なアドオンを一度削除してから読み込み直す必要があります。署名済みの通常インストールされたアドオンではこの制約は当てはまらないはずです。)
